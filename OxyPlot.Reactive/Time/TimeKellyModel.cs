...
namespace OxyPlot.Reactive.Time
{
    ...
    public class TimeKellyModel<TKey> : TimeModel<TKey, IKellyPoint<TKey>>
    {
        ...
        protected override IKellyPoint<TKey> CreatePoint(IKellyPoint<TKey> xy0, IKellyPoint<TKey> xy)
        {
            ...
            var current = xy.Odds > 0 ? xy.UnitProfit * kellyModel.KellyProfits.Sum * (kellyModel.KellyDictionary.GetValueOrDefault(xy.KellyKey?? string.Empty).Pipe(a => a == default ? 0.01 : a)) / xy.Odds : 0;
    ...
    public class Time2KellyModel<TKey> : TimeModel<TKey, IKellyPoint<TKey>>, IObserver<double>
    {
        ...
        public Time2KellyModel(PlotModel model, IEqualityComparer<TKey> comparer = null, IScheduler scheduler = null) : base(model, comparer, scheduler: scheduler)
        {
        }
